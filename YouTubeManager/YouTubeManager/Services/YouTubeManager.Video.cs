    ...
    public partial class YouTubeService
    {
        public async Task<Video> GetVideoAsync(string videoId)
        {
            ...
            if (!ValidateVideoId(videoId))
                throw new ArgumentException($"Invalid video ID {videoId}", nameof(videoId));
            ...
            var description = watchPage.QuerySelector("p#eow-description").Text();
            var likes = watchPage.QuerySelector("button.like-button-renderer-like-button").Text()
                .StripNonDigit().ParseLongOrDefault();
            var dislikes = watchPage.QuerySelector("button.like-button-renderer-dislike-button").Text()
                .StripNonDigit().ParseLongOrDefault();

            var statistics = new Statistics(views, likes, dislikes);
            var thumbnails = new Thumbnails(videoId);

            return new Video(videoId, title, description,
                author, authorId, uploadDate, duration,
                keywords, thumbnails, statistics);
        }

        private async Task<IHtmlDocument> GetVideoWatchPageAsync(string videoId)
        {
            var raw = await GetRawVideoWatchPageAsync(videoId).ConfigureAwait(false);
            return await new HtmlParser().ParseAsync(raw).ConfigureAwait(false);
        }

        private async Task<string> GetRawVideoWatchPageAsync(string videoId)
        {
        ...
        private async Task<string> GetRawVideoInfoAsync(string videoId, string el = "", string sts = "")
        {
            var eurl = $"https://youtube.googleapis.com/v/{videoId}".UrlDecode();
